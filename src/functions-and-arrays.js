...
function averageNumbers(numbersAvg) {
  ...
  let final = 0;
  for (const each of numbersAvg) {
    final += each;
  }
  let finalTwo = final / numbersAvg.length;
...
function averageWordLength(wordsArr) {
  ...
  let newCount = count / wordsArr.length;
...
function uniquifyArray(wordsUnique) {
  ...
  let uniqueChars = [...new Set(wordsUnique)];
...
function howManyTimes(wordsCount, wordToSearch) {
  ...
  for (let i = 0; i < wordsCount.length; i++) {
    if (wordsCount[i].includes(wordToSearch) === true) {
      wordCount++;
    }
  }
  return wordCount;
